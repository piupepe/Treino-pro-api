...
if not ANTHROPIC_API_KEY:
    ...
    2. Adicione: ANTHROPIC_API_KEY=sk-ant-xxxxx
    3. Onde xxxxx é sua chave real do console.anthropic.com
    
    OU no terminal:
    Windows CMD: set ANTHROPIC_API_KEY=sk-ant-xxxxx
    PowerShell: $env:ANTHROPIC_API_KEY="sk-ant-xxxxx"
...
if __name__ == '__main__':
    ...
    🎯 Webhook (Receber dados):
       POST http://localhost:5000/webhook/session
       Body: JSON da sessão
    ...
    📊 Dados:
       ...
       GET http://localhost:5000/api/latest-analysis
    
    🧪 Teste:
       POST http://localhost:5000/test/simulate-session
    
    ═════════════════════════════════════════════════════════
    
    Para usar no app:
    1. Configure URL webhook: http://SEU_IP:5000/webhook/session
    2. App enviará dados automaticamente após cada treino
    3. Agente Claude analisará em tempo real
    4. Feedback aparecerá no console
    
    ═════════════════════════════════════════════════════════
    """)
    
    # Rodar com reloader desativado em produção
    app.run(
        host='0.0.0.0',
        port=5000,
        debug=True,
